  ...
  struct
    ...
    *)
    ...
    let rec eval env (st, i, o) op =
        ...
        | Assign (v, e)   -> (State.update v (Expr.eval st e) st, i, o)
        | If (e1, e2, e3) -> if (Expr.eval st e1) != 0 then eval env (st, i, o) e2 else eval env (st, i, o) e3
        | While (e1, e2)  -> if toBool (Expr.eval st e1) then eval env (eval env (st, i, o) e2) op else (st, i, o)
